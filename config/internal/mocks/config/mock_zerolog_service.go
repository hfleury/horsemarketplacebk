...
func (m *MockLogging) GetLoggerFromContext(c context.Context) config.Logging {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetLoggerFromContext", c)
	ret0, _ := ret[0].(config.Logging)
	return ret0
}
...
func (m *MockLogging) Log(ctx context.Context, level config.LogLevel, msg string, fields map[string]any) {
	m.ctrl.T.Helper()
