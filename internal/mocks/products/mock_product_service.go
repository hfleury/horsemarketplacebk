...
func (m *MockProductService) Search(ctx context.Context, query string, categoryID string, fieldMap map[string]string, page, limit int) (*models.PaginatedProducts, error) {
	args := m.Called(ctx, query, categoryID, fieldMap, page, limit)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*models.PaginatedProducts), args.Error(1)
