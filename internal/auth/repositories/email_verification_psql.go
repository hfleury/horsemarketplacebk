...
func (er *EmailVerificationRepoPsql) Create(ctx context.Context, ev *models.EmailVerification) (*models.EmailVerification, error) {
	...
	err := er.psql.QueryRow(ctx, `INSERT INTO auth.email_verifications (user_id, verification_token, email, requested_at, expires_at) VALUES ($1,$2,$3,$4,$5) RETURNING id, user_id, verification_token, email, is_verified, requested_at, expires_at, created_at, updated_at`, ev.UserId, ev.VerificationToken, ev.Email, ev.RequestedAt, ev.ExpiresAt).Scan(
		&ev.Id,
		&ev.UserId,
...
func (er *EmailVerificationRepoPsql) SelectByToken(ctx context.Context, token string) (*models.EmailVerification, error) {
	query := `SELECT id, user_id, verification_token, email, is_verified, requested_at, expires_at, created_at, updated_at FROM auth.email_verifications WHERE verification_token = $1 LIMIT 1`
...
func (er *EmailVerificationRepoPsql) MarkVerified(ctx context.Context, token string) error {
	...
	_, err := er.psql.Execute(ctx, `UPDATE auth.email_verifications SET is_verified = $2, updated_at = NOW() WHERE verification_token = $1`, token, true)
	if err != nil {
...
func (er *EmailVerificationRepoPsql) GetLatestByEmail(ctx context.Context, email string) (*models.EmailVerification, error) {
	query := `SELECT id, user_id, verification_token, email, is_verified, requested_at, expires_at, created_at, updated_at FROM auth.email_verifications WHERE email = $1 ORDER BY requested_at DESC LIMIT 1`
	ev := &models.EmailVerification{}
